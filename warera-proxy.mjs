...
async function getBattles(q) {
  ...
  const drzava = (id) => cIdx.get(id) || (id ? { id, name: null, code: null, flag: "" } : null);

  return asList(raw).map((b) => {
    const att = b.attacker || {}, def = b.defender || {};
    const attId = b.attackerCountry || att.country || att._id || null;
    const defId = b.defenderCountry || def.country || def._id || null;
    // regija je ugnijezdena u defender/attacker, ne na vrhu
    const regId = def.region || b.defenderRegion || att.region || b.region || b.regionId || null;

    // kompaktan sazetak umjesto cijelog sirovog objekta (koji je golem)
    const cr = b.currentRound || {};
    const stranu = (s) => s ? { steta: s.damages ?? 0, bodovi: s.points ?? 0, udarci: s.hitCount ?? 0 } : null;
    const a = stranu(cr.attacker), d = stranu(cr.defender);

    return {
      id: b._id,
      url: b._id ? `https://app.warera.io/battle/${b._id}` : null,
      isActive: b.isActive !== false,
      napadac: attId ? drzava(attId) : null,
      branitelj: defId ? drzava(defId) : null,
      regija: regId ? { id: regId, name: rIdx.get(regId) || null } : null,
      naziv: b.name || null,
      tip: b.type || null,
      runda: cr.number ?? null,
      stanje: (a || d) ? { napadac: a, branitelj: d, vodi: (a && d) ? (a.bodovi > d.bodovi ? "napadac" : a.bodovi < d.bodovi ? "branitelj" : "izjednaceno") : null } : null,
    };
  });
}
